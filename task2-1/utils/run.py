...
def create_visit_order(num_casinos=3, visits_per_casino=24, seed=None):
...
async def run_experiment_task1(labels_csv=None,
    ...
    """
    Run an experiment based on the Palminteri et al. (2017) Factual Learning paradigm.
    - 72 trials total per subject
    - 3 casinos visited 24 times each:
      - Casino 1: Symmetric [0.5, 0.5]
      - Casino 2: Asymmetric [0.75, 0.25]
      - Casino 3: Asymmetric Reversed [0.25, 0.75]
    - Rewards: +1 for win, -1 for loss.
    - Feedback: Only the outcome of the chosen option is revealed.
    ...
    for trial_idx, c in enumerate(visit_order, 1):
        ...
        logger.info(f"OUTCOME: Machine {chosen_label} delivered {reward} points (prob of no loss: {prob_of_no_loss:.2f})")

        # Create the outcome text that will be shown via prompt history / added to conversation history
        outcome_line = f"Machine {chosen_label} in Casino {c} delivered {reward} points."

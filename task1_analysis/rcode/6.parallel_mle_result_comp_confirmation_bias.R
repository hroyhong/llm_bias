...
pa<-function(li){
  ...
  LogL<-function(pp){
    # With this:
    alpha1 <- pp[1] # Alpha for Positive PE Chosen
    alpha2 <- pp[2] # Alpha for Negative PE Chosen
    ...
    for (i in 1:nrow(dd)){
      ...
      if (casino==2){
        ...
        if(pe1>0){
          value2[choose]<-value2[choose] + alpha1 * pe1 ## Chosen PE > 0
        }
        if(pe1<0){
          value2[choose]<-value2[choose] + alpha2 * pe1 ## Chosen PE < 0
        }
        ...
        if (pe_unchosen > 0) {
          value2[unchosen_index] <- value2[unchosen_index] + alpha2 * pe_unchosen # Unchosen PE > 0
        }
        if (pe_unchosen < 0) {
          value2[unchosen_index] <- value2[unchosen_index] + alpha1 * pe_unchosen # Unchosen PE < 0
        }
      ...
      if (casino==3){
        ...
        if(pe1>0){
          value3[choose]<-value3[choose] + alpha1 * pe1 ## Chosen PE > 0
        }
        if(pe1<0){
          value3[choose]<-value3[choose] + alpha2 * pe1 ## Chosen PE < 0
        }
        ...
        if (pe_unchosen > 0) {
          value3[unchosen_index] <- value3[unchosen_index] + alpha2 * pe_unchosen # Unchosen PE > 0
        }
        if (pe_unchosen < 0) {
          value3[unchosen_index] <- value3[unchosen_index] + alpha1 * pe_unchosen # Unchosen PE < 0
        }
      }
  ...
  estimates_school = DEoptim(LogL,lower =c(0,0,0),upper=c(1,1,40),control = DEoptim.control(trace = FALSE,NP = 80,itermax = 250))
  dd1<-summary(estimates_school)
  a<-dd1$optim$bestmem[1] # alpha_pos_c
  b<-dd1$optim$bestmem[2] # alpha_neg_c
  e<-dd1$optim$bestmem[3] # tau

...
EXPERIMENT_CONFIG = {
    ...
    # Reward structure
    "rewards": {
        "win": 0,   # Reward for winning
        "loss": -1  # Reward for losing (no loss)
    }
}

# System message for LLM
SYSTEM_MESSAGE = """You are going to visit four different casinos (named 1, 2, 3, and 4) 24 times each. Each casino owns two slot machines which stochastically give you either 0 points (no loss) or -1 points (a loss) with different loss probabilities. Your goal is to minimize the total points lost within 96 visits.
